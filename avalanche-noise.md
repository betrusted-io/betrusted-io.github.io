...
Avalanche breakdown is essentially a miniature particle accelerator,
where electrons that enter the PN junction depletion region (through
mechanisms that include thermal noise) and are accelerated across an
electrical field, to the point where new electron-hole pairs are
generated when these high-energy electrons collide with atoms,
creating an amplification cascade with low reproduceability.

An approximate analogy is an inflatable pool filled with water. The
height of the pool is the potential barrier of the reverse-biased PN
junction. A hose feeding water into the pool represents a constant
current of electrons. The volume of the pool can be thought of as the
depletion capacitance, that is, the virtual capacitor created by the
region of the junction that is void of carriers due to natural drift
and diffusion effects. As the water trickles into the pool, eventually
at some point in time, a meniscus forms, and random events, such as
ripples formed on the surface due to wind, will eventually cause the
meniscus to flow over the edge. The water flowing over the edge pushes
down on the inflatable side of the pool, causing more water to flow,
until a large amount flows and the inflatable side of the pool can
snap back into its original conformation, thus restarting the cycle of
filling, meniscus, and breakdown. The unpredictability of when and
where the breakdown might happen, and how much water flows out during
the event, is analagous to the entropy generated by the avalanche
effect in a PN junction. 

The elecrtical characteristic of avalanche noise biased by a constant
current source is a "sawtooth" waveform: a semi-linear ramp up in
voltage as the capacitance of the depletion region charges to the
point where the electric field becomes large enough to initiate the
cascade, and then a sharp drop off in voltage as the cascade rapidly
increases the conduction of the junction. The cascade then abruptly
halts once the field is no longer strong enough to sustain the cascade
effect, leading to a subsequent cycle of charging the depletion
capacitance.
